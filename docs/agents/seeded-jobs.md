...
The following facts are source-derived from the ePublisher trunk; the Task Scheduler behavior was observed in the Administrator on 2026-09-23 (Run creates the job's `waj` task and opens its properties), the path handling was not. `Publish/Core/Deployment/FolderDeployTarget.cs` uses `<Configuration Value>` verbatim, without full-path resolution or environment-variable expansion; it creates the folder if missing and cleans it only when `cleanOutput="True"`. `Automap/Core/JobManager.cs` and `Automap/Core/TaskSchedulerManager.cs` show that the Administrator runs every job, including Run, through Windows Task Scheduler with an action of the executable plus the quoted absolute `.waj` path, no working directory, so the process starts in `C:\Windows\System32`; the CLI never sets a working directory. Because `Documents` varies per user and may be OneDrive-redirected, no fixed absolute path works.

## What the Administrator does with output

Confirmed in the GUI on 2026-09-23 (AutoMap 2026.1.4761) for the cases the trial exercises, the rest source-derived: `Automap/AutomapUI/UI/JobsForm.cs` and `Publish/Core/Deployment/DeployPreview.cs` indicate that Explore Output and Preview Output in Browser are one dropdown each, with one entry per target that has a deploy destination, captioned by target name. A target with `build="False"` and a destination is listed disabled; a target without a destination is not listed; a job with no destination shows one `no deploy targets found` entry. Neither command opens Staging.
...
For a Composition job, the same commands have one entry named by the composition `Destination`; Preview opens `index.html` in that folder. The Administrator scans `Jobs\<sub-folder>\*.waj|*.wacj` live, and a job log is `<name>-log.txt` beside the job file. Preview Output in Browser opened each site in the default browser; the single job's submenu listed only Web Help, and the composition's submenu listed only its destination, Quantum Sync Site.
...
Member builds spawned by a Composition job do not update the members' own rows in the job list (their Last Run stays as it was), because the composition builds them as child processes rather than through their scheduled tasks. They also do not inherit `--stagingdir`; they stage under the product-default Staging folder, `Documents\WebWorks ePublisher AutoMap\Staging\<member name>\`. This is irrelevant to a trial user in the Administrator but matters for CLI rehearsals on a mock drive, so the rehearsal script removes only the folders it created there.
...
The New Job wizard's Target Configuration page will not leave the Info tab for a built target until a destination is chosen ("Please select an output location."), so a publishing job gets its Folder destination before any per-target override; the Edit Job window additionally offers a Job Deploy Destinations tab for inline destinations.
...
For hand-staging on a real machine for screenshots in #35, copy Evaluation according to `docs/agents/extraction-layout.md`, then run `python scripts/stage_seeded_jobs.py "<Documents>\WebWorks ePublisher AutoMap"`. This adds the three job folders to the live Jobs folder; remove them after capture. For the #35 capture the seeded state was staged under a neutral product folder (`C:\Users\Public\Documents\WebWorks ePublisher AutoMap`) with the Administrator's Job and Staging folder preferences pointed there for the session, which keeps the live job list and personal paths out of the screenshots; the preferences were restored afterwards.
