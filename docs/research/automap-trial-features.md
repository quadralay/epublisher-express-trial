...
## Third verification pass (2026-09-23): handoff hook points

- Express paths.json Evaluation entry is at 142-144 (the note's "143-145" above drifted by one); Express `install_files.nsh` is 390-392 (not 388-390). AutoMap's Help anchors: x64 `paths.json:149-151`, x86 `paths_x86.json:125-127`; `collector.py` regenerates the `.nsh` files from them.
- `DefaultDirectoriesCreated` lives on the shared `PublisherPreferences` (`Publish/Core/PublisherPrefences.cs:532-550`, filename spelled without the second e). AutoMap constructs with `base(true)` (`AutomapApplication.cs:275-276`), so its `Preferences.xml` is machine-wide under ProgramData (`AutomapApplication.cs:48`; `PublisherPrefences.cs:619-621`), and the installer's `--register` run (`AutomapUI.cs:151-171`) flips it once per install. Express uses `base(false)` (`RedstoneApplication.cs:566`), per-user. Consequence: the stub at `AutomapApplication.cs:463-474` cannot serve as a per-user first-launch hook; the handoff spec hooks the Administrator startup instead.
- `AdminUI.prefs` is not per-user either: `AutomapUIPrefs.Prefs` (`AutomapUIPrefs.cs:56-60`) roots it at `AutomapPreferences.ApplicationPreferencesDirectoryPath` (`AutomapPreferences.cs:156-167`), the same ProgramData folder; `LocalAppDataPathStore` (`Common/Preferences.cs:312`) is a plain file-backed store despite its name. A per-user file has to be rooted with the public static `PublisherPreferences.DetermineApplicationPreferencesRootPath(false, ...)` (`PublisherPrefences.cs:613`), and stores only persist on `Flush` (`Common/Preferences.cs:465-476`).
- Administrator startup order: the constructor creates the Jobs/Staging folders (`AutomapUI.cs:58-71`) and `new JobsForm()` (`:77`) scans the Jobs folder at construction (`JobsForm.cs:129` -> `JobManager.SetJobDirectory` -> `LoadAllJobs`, `JobManager.cs:198-216`); the license check runs later, on first `Application.Idle` (`LicensingHandler`, `AutomapUI.cs:101-124`). AutoMap's own `install_files.nsh` Help pair is at 393-394.
- The stub is in `AutomapApplication.cs`, not `AutomapPreferences.cs`; `AutomapPreferences.cs:93-109` only defines the Jobs/Staging defaults under the literal `WebWorks ePublisher AutoMap` product folder.
- Express has no Help-menu Trial Guide item: its link is the Start Page "At a Glance" entry (`RedstoneApplication.cs:293-305`, caption `AtAGlance_EvaluationGuide`, `strings.resx:310-311`), and that list is shown only when `EvaluationOnly()` is false (`:327-334`).
- `ExtractEvaluationWEZ` is `Publish/Core/PublisherApplication.cs:391-425` (comment 391-400, body 402-425); it creates the target only when absent (a OneDrive-placeholder workaround) and its catch at 419-422 is empty ("Silently fail!"). Express does not log extraction failures.
- Express's `Reset Evaluation Materials` (`ApplicationPrefDialog.cs:749-760`) deletes and re-extracts without a confirmation prompt.
- Evaluation mode does constrain output: an 8-document generation cap (`Publish/Core/Engine/Engine.cs:1531-1558`), a watermark on Ghostscript-rendered images (`Engine.cs:562-579`) and blocked XSL exec (`Engine.cs:552-560`). This refines "Gap 2 CLOSED" above (no format-level watermark call) rather than reversing it.
- AutoMap does not reuse Express's `.licinfo` (`PublisherPrefences.cs:868-878`); keys are per application ID, AutoMap is ID 21 (`AutomapApplication.cs:535-538`).
