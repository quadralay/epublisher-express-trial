...
| [3.1][release-3-1] | June 16, 2026 | Per-folder retention, offline status badges, Sync Now on Linux |
| [3.0][release-3-0] | March 10, 2026 | Offline Access for shared folders, idle-aware downloads, faster recovery from network interruptions |
...
- **Sync Now on Linux** - The **Sync Now** command is now available on Linux from the application window and the Ctrl+Shift+S shortcut, including on desktops without a system tray.

### Fixes

- Files matching an **Ignore file patterns** entry, such as temporary files, are no longer uploaded when they are created during an active sync.
- Connection errors through a SOCKS5 proxy that requires authentication now identify the proxy instead of reporting a generic server error.
- Proxy credentials are now requested once per session when **Proxy type** is set to HTTP. Previously the prompt appeared twice.
- Conflict copies created while offline now receive a timestamp in the device's local time instead of UTC.
...
- **Audit logging** does not yet record changes to shared-link expiration dates. Resolved in 3.2.
<!-- /condition -->
...
- **Offline Access for shared folders** - Folders that other people share with you can now be marked for offline availability. Queued changes and conflict detection work the same way as for your own folders.
- **Resume on metered connections** - When automatic sync pauses on a metered connection, you can now resume it for the current session from the system tray menu instead of switching to manual mode.
- **Idle-aware downloads** - Downloads from other devices now follow the same scheduling as uploads: throttled while you are actively working and accelerated when the device is idle.
- **Faster recovery from network interruptions** - After a dropped connection, the WebSocket connection on port 8443 reconnects automatically and replays the change notifications the device missed, so files no longer wait for the next full scan of the sync folder.

### Fixes

- Uploads interrupted by system sleep now resume from the last completed chunk instead of restarting.
- **Start on login** is now honored on Windows when the application was installed for all users.
- Comparing versions side-by-side no longer fails for files larger than 100 MB.
...
- On Linux desktops without a system tray, the $ProductName; icon does not appear, so **Sync Now** cannot be triggered from the tray. Use the web interface instead. Resolved in 3.1.
- Proxy credentials are requested twice when **Proxy type** is set to HTTP and the proxy requires authentication. Resolved in 3.1.
