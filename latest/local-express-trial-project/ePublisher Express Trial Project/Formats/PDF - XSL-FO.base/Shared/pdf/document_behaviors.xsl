...
>
 ...
 <xsl:template match="wwdoc:Table" mode="wwmode:behavior">
  ...
  <xsl:if test="$VarGenerateOutput">
   <!-- Start new page sequence (Split Priority) -->
   <!--                                          -->
   <xsl:variable name="VarStartNewPageSequenceOption" select="$VarContextRule/wwproject:Options/wwproject:Option[@Name = 'start-new-page-sequence']/@Value" />
   <xsl:variable name="VarStartNewPageSequence" select="$VarStartNewPageSequenceOption = 'true'" />
   <xsl:variable name="VarSplitPriority">
    <xsl:choose>
     <xsl:when test="$VarStartNewPageSequence">
