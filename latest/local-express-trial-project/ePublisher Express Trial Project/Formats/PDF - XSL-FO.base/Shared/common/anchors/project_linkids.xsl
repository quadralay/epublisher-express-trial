...
>
 ...
 <xsl:template match="/">
  <wwfiles:Files version="1.0">
   ...
   <xsl:for-each select="$GlobalFiles[1]">
    <!-- Load project links -->
    <!--                    -->
    <xsl:variable name="VarLinksFile" select="key('wwfiles-files-by-type', $ParameterLinksType)[1]" />
    ...
    <xsl:for-each select="$VarFilesByType">
     ...
     <xsl:if test="not($VarUpToDate)">
      <xsl:variable name="VarResultAsTreeFragment">
       ...
       <!-- Add linkids -->
       <!--             -->
       <xsl:apply-templates select="$VarDocumentLinks" mode="wwmode:linkids">
 ...
 <xsl:template match="/" mode="wwmode:linkids">
  <xsl:param name="ParamLinks" />

  <wwlinks:Anchors>
   <xsl:apply-templates mode="wwmode:linkids">
 ...
 <xsl:template match="wwlinks:Anchor" mode="wwmode:linkids">
  <xsl:param name="ParamLinks" />
  ...
  <xsl:for-each select="$ParamLinks[1]">
   <xsl:variable name="VarParagraphsWithID" select="key('wwlinks-paragraphs-by-id', $ParamAnchor/@value)" />

   <xsl:for-each select="$VarParagraphsWithID">
    <xsl:variable name="VarParagraphWithID" select="." />

    <wwlinks:Anchor value="{$VarParagraphWithID/@linkid}" />
   </xsl:for-each>
  </xsl:for-each>
 ...
 <xsl:template match="text() | comment() | processing-instruction()" mode="wwmode:linkids">
  <xsl:param name="ParamLinks" />
