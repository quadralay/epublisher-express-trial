...
>
   ...
   <xsl:for-each select="$VarProjectGroups">
    ...
    <xsl:if test="not(wwprogress:Abort())">
     ...
     <xsl:for-each select="$GlobalFiles[1]">
      ...
      <xsl:variable name="VarUpToDate" select="wwfilesext:UpToDate($VarPath, string(count($VarGroupFilesOfType)), $VarProjectGroup/@GroupID, '', $GlobalActionChecksum)" />
      <xsl:if test="not($VarUpToDate)">
       <xsl:variable name="VarResultAsTreeFragment">
        <!-- Merge document anchors -->
        <!--                        -->
        <wwlinks:Anchors>
         <xsl:for-each select="$VarGroupFilesOfType">
          ...
          <!--          -->
          <xsl:if test="not(wwprogress:Abort())">
           <!-- Load document anchors -->
           <!--                       -->
           <xsl:variable name="VarDocumentAnchors" select="wwexsldoc:LoadXMLWithoutResolver($VarFile/@path, false())" />

           <!-- Copy anchors -->
           <!--              -->
           <xsl:apply-templates select="$VarDocumentAnchors" mode="wwmode:copy-anchors" />
          </xsl:if>
         </xsl:for-each>
        </wwlinks:Anchors>
       </xsl:variable>
      ...
      <!--          -->
      <xsl:if test="not(wwprogress:Abort())">
       <wwfiles:File path="{$VarPath}" type="{$ParameterType}" checksum="{wwfilesystem:GetChecksum($VarPath)}" projectchecksum="{string(count($VarGroupFilesOfType))}" groupID="{$VarProjectGroup/@GroupID}" documentID="" actionchecksum="{$GlobalActionChecksum}">
        <xsl:for-each select="$VarGroupFilesOfType">
 ...
 <xsl:template match="/" mode="wwmode:copy-anchors">
  <xsl:apply-templates mode="wwmode:copy-anchors" />
 </xsl:template>


 <xsl:template match="wwlinks:Anchor" mode="wwmode:copy-anchors">
  <!-- Preserve -->
  <!--          -->
  <xsl:copy>
   <xsl:copy-of select="@*" />
 ...
 <xsl:template match="text() | comment() | processing-instruction()" mode="wwmode:copy-anchors">
  <!-- Nothing to do -->
  <!--               -->
 </xsl:template>
</xsl:stylesheet>
