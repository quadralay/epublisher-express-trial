...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
                              ...
                              exclude-result-prefixes="xsl msxsl wwmode wwfiles wwdoc wwsplits wwtoc wwproject wwprogress wwlog wwfilesystem wwuri wwstring wwprojext wwexsldoc wwstageinfo"
>
    ...
    <xsl:if test="$VarGroupPosition &gt; $VarLastGroupPosition">
     ...
     <xsl:if test="not(wwprogress:Abort())">
      <xsl:for-each select="$ParamFiles[1]">
       ...
       <!-- TOC Data -->
       <!--          -->
       <xsl:variable name="VarTOCDataPath">
        <xsl:variable name="VarFilesGroupTOCData" select="key('wwfiles-files-by-type', $ParamTOCDataType)[@groupID = $VarProjectGroup/@GroupID][1]" />
        <xsl:variable name="VarFilesProjectTOCData" select="key('wwfiles-files-by-type', $ParamTOCDataType)[@groupID = ''][1]" />

        <!-- Allow project-wide TOC data for PDF XSL-FO -->
        <!--                                            -->
        <xsl:choose>
         <xsl:when test="count($VarFilesGroupTOCData) = 1">
          <xsl:value-of select="$VarFilesGroupTOCData/@path" />
         </xsl:when>

         <xsl:otherwise>
          <xsl:value-of select="$VarFilesProjectTOCData/@path" />
         </xsl:otherwise>
        </xsl:choose>
       </xsl:variable>
       <xsl:variable name="VarTOCData" select="wwexsldoc:LoadXMLWithoutResolver($VarTOCDataPath, false())" />
     ...
     <xsl:if test="($ParamSegmentDocumentStartPosition &lt;= $VarSplitDocumentStartPosition) and (($ParamSegmentDocumentEndPosition = -1) or ($VarSplitDocumentStartPosition &lt;= $ParamSegmentDocumentEndPosition))">
      ...
      <xsl:if test="$VarSplitPosition &gt; $VarLastSplitPosition">
       ...
       <xsl:for-each select="$ParamTOCData[1]">
        ...
        <xsl:variable name="VarFirstTOCEntryOnSplitPage" select="$VarDocumentTOCEntries[@documentposition &gt;= $VarSplit/@documentstartposition and @documentposition &lt;= $VarSplit/@documentendposition][1]" />
        <xsl:variable name="VarBreadcrumbPossibleTOCEntryID">
         <xsl:choose>
          <xsl:when test="count($VarFirstTOCEntryOnSplitPage) = 0">
           <!-- Fall back to first TOC entry on page -->
           <!--                                      -->
           <xsl:variable name="VarFirstPreviousTOCEntryInSameDocument" select="$VarDocumentTOCEntries[@documentposition &lt;= $VarSplit/@documentstartposition][1]" />

           <xsl:choose>
            <xsl:when test="count($VarFirstPreviousTOCEntryInSameDocument) = 1">
             <xsl:value-of select="$VarFirstPreviousTOCEntryInSameDocument/@id" />
            </xsl:when>

            <xsl:otherwise>
             <!-- Fall back to first TOC entry -->
             <!--                              -->
             <xsl:value-of select="$ParamTOCData/wwtoc:TableOfContents/wwtoc:Entry[1]/@id" />
            </xsl:otherwise>
           </xsl:choose>
          </xsl:when>

          <xsl:otherwise>
           <xsl:value-of select="$VarFirstTOCEntryOnSplitPage/@id" />
          </xsl:otherwise>
         </xsl:choose>
        </xsl:variable>
        <xsl:variable name="VarBreadcrumbTOCEntry" select="$VarDocumentTOCEntries[@id = $VarBreadcrumbPossibleTOCEntryID][1]" />

        <!-- Content -->
        <!--         -->
        <xsl:variable name="VarSplitStartElement" select="$ParamDocumentSegment/wwdoc:Document/wwdoc:Content/*[@id = $VarSplit/@id]" />
        ...
        <xsl:call-template name="Page">
         ...
         <xsl:with-param name="ParamDocument" select="$ParamDocumentSegment" />
