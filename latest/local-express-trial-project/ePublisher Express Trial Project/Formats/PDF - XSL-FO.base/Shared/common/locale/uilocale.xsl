...
<xsl:stylesheet version="1.0" xmlns="urn:WebWorks-Engine-Links-Schema"
                              ...
                              exclude-result-prefixes="xsl msxsl wwmode wwlinks wwfiles wwsplits wwdoc wwproject wwlocale wwprogress wwlog wwfilesystem wwuri wwstring wwfilesext wwprojext wwexsldoc wwenv"
>
 ...
 <xsl:template match="/">
  <wwfiles:Files version="1.0">
   ...
   <xsl:variable name="VarDefaultLocalesPath" select="wwuri:AsFilePath('wwtransform:common/locale/uilocales.xml')" />
   <xsl:variable name="VarFormatLocalesPath" select="wwuri:AsFilePath('wwformat:Transforms/uilocales.xml')" />
   ...
   <xsl:variable name="VarProjectChecksum">
    ...
    <xsl:text>:</xsl:text>
    <xsl:value-of select="wwenv:CurrentUILocale()" />
    <xsl:text>:</xsl:text>
   ...
   <xsl:if test="not($VarUpToDate)">
    ...
    <xsl:variable name="VarLocaleAsXML">
     <xsl:call-template name="Locale-Select">
      ...
      <xsl:with-param name="ParamRequestedLocaleName" select="wwenv:CurrentUILocale()" />
     </xsl:call-template>
    </xsl:variable>
