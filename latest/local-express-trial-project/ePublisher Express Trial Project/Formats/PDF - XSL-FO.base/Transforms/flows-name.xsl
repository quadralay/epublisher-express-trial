      ...
      <xsl:for-each select="$GlobalFiles[1]">
       ...
       <xsl:if test="not($VarUpToDate)">
        <xsl:variable name="VarResultAsXML">
         <flo:Flows>
          <xsl:call-template name="Flows-Name">
 ...
 <xsl:template name="Flows-Name">
  ...
  <xsl:for-each select="$ParamFlows/flo:Flows/flo:Flow">
   ...
   <xsl:variable name="VarFlowName">
    <xsl:value-of select="wwstring:ToLower(wwstring:CSSClassName($VarFlow/@name))" />
    <xsl:text>_</xsl:text>
    <xsl:value-of select="position()" />
   </xsl:variable>

   <flo:Flow name="{$VarFlowName}" stylename="{$VarFlow/@name}">
    <xsl:copy-of select="$VarFlow/*" />
   </flo:Flow>
  </xsl:for-each>
 </xsl:template>
</xsl:stylesheet>
