...
>
 ...
 <xsl:template match="/">
  <wwfiles:Files version="1.0">
   <xsl:if test="wwprojext:GetFormatSetting('generate-group-result') = 'true'">
       ...
       <xsl:for-each select="$GlobalFiles[1]">
        ...
        <xsl:if test="not(wwprogress:Abort())">
         <xsl:for-each select="$GlobalFiles[1]">
          ...
          <!-- Record files -->
          <!--              -->
          <wwfiles:File path="{$VarPath}" type="{$ParameterType}" checksum="{wwfilesystem:GetChecksum($VarPath)}" projectchecksum="{$GlobalProject/wwproject:Project/@ChangeID}" groupID="{$VarProjectGroup/@GroupID}" documentID="" actionchecksum="{$GlobalActionChecksum}">
 ...
 <xsl:template name="Flows-Flows">
  ...
  <xsl:for-each select="$ParamSplits">
   ...
   <xsl:variable name="VarPageOutputPath" select="wwfilesystem:Combine(wwprojext:GetTargetDataDirectoryPath(), $VarSplit/@groupID, $VarSplit/@documentID, concat('page_group_output_', $VarSplit/@position, '.xml'))" />
