...
>
 ...
 <xsl:variable name="GlobalValidCSSProperties" select="wwexsldoc:LoadXMLWithoutResolver('wwtransform:html/css_properties.xml', false())" />
   ...
   <xsl:for-each select="$GlobalValidCSSProperties[1]">
    <xsl:for-each select="key('wwprops-property-by-name', $VarProperty/@Name)[1]">
     <xsl:choose>
      ...
      <!-- cursor -->
      <!--        -->
      <xsl:when test="$VarProperty/@Name = 'cursor'">
       <xsl:choose>
        <xsl:when test="$VarProperty/@Value = 'pointer'">
         <!-- Emit both pointer and hand for IE compatibility -->
         <!--                                                 -->
         <wwproject:Property Name="cursor" Value="pointer" />
         <wwproject:Property Name="cursor" Value="hand" />
        </xsl:when>
      ...
      <!-- font-family -->
      <!--             -->
      <xsl:when test="$VarProperty/@Name = 'font-family'">
       <xsl:choose>
        <xsl:when test="($VarProperty/@Value = 'Symbol') or ($VarProperty/@Value = 'Zapf Dingbats')">
         <!-- Suppress for the sake of Internet Explorer -->
         <!--                                            -->
        </xsl:when>
