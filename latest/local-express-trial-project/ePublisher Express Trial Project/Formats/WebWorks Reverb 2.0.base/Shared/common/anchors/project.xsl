 ...
 <xsl:template match="/">
  <wwfiles:Files version="1.0">
   ...
   <xsl:for-each select="$GlobalFiles[1]">
    <xsl:variable name="VarGroupFilesOfType" select="key('wwfiles-files-by-type', $ParameterDependsType)" />
    ...
    <xsl:variable name="VarUpToDate" select="wwfilesext:UpToDate($VarPath, string(count($VarGroupFilesOfType)), '', '', $GlobalActionChecksum)" />
      ...
      <wwlinks:Anchors>
       <xsl:for-each select="$VarGroupFilesOfType">
        ...
        <xsl:if test="not(wwprogress:Abort())">
         <!-- Load group anchors -->
         <!--                    -->
         <xsl:variable name="VarGroupAnchors" select="wwexsldoc:LoadXMLWithoutResolver($VarFile/@path, false())" />

         <!-- Copy anchors -->
         <!--                -->
         <xsl:apply-templates select="$VarGroupAnchors" mode="wwmode:copy-anchors" />
        </xsl:if>
       </xsl:for-each>
      </wwlinks:Anchors>
     </xsl:variable>
    ...
    <!--          -->
    <xsl:if test="not(wwprogress:Abort())">
     <wwfiles:File path="{$VarPath}" type="{$ParameterType}" checksum="{wwfilesystem:GetChecksum($VarPath)}" projectchecksum="{string(count($VarGroupFilesOfType))}" groupID="" documentID="" actionchecksum="{$GlobalActionChecksum}">
      <xsl:for-each select="$VarGroupFilesOfType">
