// Copyright (c) 2018-2025 Quadralay Corporation.  All rights reserved.
//
// ePublisher 2025.1
//
...
function Menu_Show() {
  ...
  var presentation_div, menu_toggle_button;
  ...
  // Update aria-expanded state on menu toggle button
  menu_toggle_button = document.querySelector('.ww_behavior_menu');
  if (menu_toggle_button) {
    menu_toggle_button.setAttribute('aria-expanded', 'true');
  }
...
function Menu_Hide() {
  ...
  menu_toggle_button = document.querySelector('.ww_behavior_menu');
  if (menu_toggle_button) {
    menu_toggle_button.setAttribute('aria-expanded', 'false');
  }
...
function Menu_Display(param_setup) {
  'use strict';
...
function Menu_ResizeMenu(height) {
  ...
  if (menu_element !== null) {
    ...
    if (nav_buttons_element !== null) {
      ...
      this.ResizeAssistant(content_height);
    }
  }
};
...
function Menu_ResizeAssistant(height) {
  'use strict';
  var assistant_title_element, assistant_content_element,
    assistant_title_height, calculated_height;

  calculated_height = 0;

  assistant_title_element = document.getElementById("assistant_title");
  assistant_content_element = document.getElementById("assistant_content");

  if (assistant_title_element && assistant_content_element) {
    assistant_title_height = parseFloat(window.getComputedStyle(assistant_title_element, null)["height"]);
    assistant_title_height = !isNaN(assistant_title_height) ? assistant_title_height : 0; // So we can do maths on it

    calculated_height = height - assistant_title_height;
    assistant_content_element.style["height"] = calculated_height + "px";
  }
};
...
function Menu_Object(param_window, param_connect) {
  ...
  this.ResizeAssistant = Menu_ResizeAssistant;
};