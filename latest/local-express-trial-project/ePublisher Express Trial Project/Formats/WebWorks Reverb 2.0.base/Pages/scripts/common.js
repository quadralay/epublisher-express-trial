...
/**
 * Set selected state for "Was this helpful?" buttons.
 * Updates CSS classes and aria-pressed attributes.
 * Shared utility used by page.js and search.js.
 *
 * @param {string} param_helpful_rating - 'yes', 'no', or undefined to reset
 */
Browser.SetHelpfulButtonState = function (param_helpful_rating) {
  'use strict';
  ...
  if (!helpful_button || !unhelpful_button) {
    return;
  }
  ...
  helpful_button.setAttribute('aria-pressed', 'false');
  unhelpful_button.setAttribute('aria-pressed', 'false');

  // Set selected state if a rating is provided
  if (param_helpful_rating === 'yes') {
    ...
    helpful_button.setAttribute('aria-pressed', 'true');
    ...
    unhelpful_button.setAttribute('aria-pressed', 'true');
  }
};

/**
 * Initialize Alt+N/P/Home keyboard shortcuts for navigation.
 * Forwards navigation actions to parent frame via Message.Post.
 * Shared utility used by page.js and search.js.
 *
 * @param {Window} param_window - The window object to attach the listener to
 */
Browser.InitNavigationKeyboardShortcuts = function (param_window) {
  'use strict';

  param_window.document.addEventListener('keydown', function (param_event) {
    var action = null;
    ...
    if (action) {
      ...
      Message.Post(param_window.parent, { 'action': action }, param_window);
    }
  });
};
 ...
 * @param {Window} param_window - The window object to attach the listener to
 */
Browser.InitRegionCycling = function (param_window) {
  'use strict';
  ...
  param_window.document.addEventListener('keydown', function (param_event) {
    ...
    Message.Post(param_window.parent, {
      'action': 'region_cycle',
      'direction': direction,
      'from': 'content'
    }, param_window);
...
Browser.DecodeHTMLEntities = function (param_string) {
  var textarea;

  textarea = document.createElement('textarea');
  textarea.innerHTML = param_string;
